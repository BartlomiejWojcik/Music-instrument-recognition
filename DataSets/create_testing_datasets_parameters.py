...
def createCsvData():
    ...
    header = header.split()
    # write header
    with open(str(path.parent) + f'/Networks/param_data_test.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(header)
    # write data for each audio
    for filename in os.listdir(str(path) + f'/IRMAS-TestingData-Part1/Part1'):
        if (".txt"  in filename):
            f = open(str(path) + f'/IRMAS-TestingData-Part1/Part1/{filename}', "r")
            genre_r = f.read()
            j = 0
            for i in range(int(len(genre_r)/5)):
                ...
                songname = str(path) + f'/IRMAS-TestingData-Part1/Part1/{filename}'
                to_append = specific_data(songname, genre)
                file = open(str(path.parent) + f'/Networks/param_data_test.csv', 'a', newline='')
                with file:
                    writer = csv.writer(file)
